...
This excellent post from [10MinuteMail](https://www.digitalsanctuary.com/10minutemail/migrating-10minutemail-from-java-to-graalvm-native.html)  looks at how they used GraalVM and Spring Boot 3 to reduce their startup time from ~30 s down to about 3 ms, and memory usage from 6.6 GB down to 1 GB, with the same throughput and CPU utilization.

== from Java 17, onward and upward

So many features in Java 21 culminate with other elements in Java 17 (and, in some cases, earlier than that!). Let's review some of those features before examining their manifestation in Java 21.
...
I rarely used the existing `switch` statement because it was clunky, and usually, there were other patterns, like the visitor pattern, that got me most of the benefits. But now there's a new switch which, if nothing else, results in an expression. So I can assign the results of the `switch` to a variable or return it.
...
Notice that we've assigned a variable to the resulting type that's now automatically downcast to the type we want. There's no need for the redundant handling of the class `Dog`.

The smart casting `instanceof` operator usage is the first real taste of pattern matching in the Java platform. The idea behind pattern matching is simple: match types and extract data from those types.
...
The enhanced `switch` expression and pattern matching are remarkable, and it makes me wonder how using Akka so many years ago would've felt using Java with this excellent new syntax. Pattern matching has an even nicer interaction when taken together with records because records - as discussed earlier - are the resumes of their components, and the compiler knows this. So, it can hoist those components into new variables, too.

You can also use this pattern-matching syntax in `if` checks.
...
In AI and algorithms, efficient mathematics is more important than ever. The new JDK has some nice improvements here, including parallel multiplication for BigIntegers and various overloads for divisions that throw an
...
<2> There's new support for parallelizing multiplication with `BigInteger` instances. Remember that this is only really useful if the `BigInteger` is in 1000s bits...

=== `Future#state`

If you're doing asynchronous programming (yes, that's still a thing, even with Project Loom, you'll be pleased to know
our old friend `Future<T>` now makes available a state object that you can switch on to see the status of the  ongoing operation..)

[source, Java]
----
include::src/test/java/bootiful/java21/FutureTest.java[]
----
<1> This returns a `state` object that lets us enumerate the submitted `Thread` states. It pairs nicely with the enhanced switch feature, too!
...
I used `HttpResponse.BodyHandlers.ofString` to get a `String` response in that example. You can get all sorts of
objects back, not just Strings. But Strings are nice because they are a great segue to another fantastic feature in Java
...
If you want to order those strings in a collection, you'll wish to order a collection. The JDK has a few of them, `LinkedHashMap`, `List`, etc., but they didn't have a common ancestor. Now they do: welcome, `SequencedCollection`! In this example, we work with a simple `ArrayList` and use the fancy new factory methods for things like a `LinkedHashSet`.
This new constructor is excellent because it does some math internally to guarantee that it won't have to rebalance ( and thus rehash everything) before you've added as many elements as you've stipulated in the constructor.
...
What do I mean by that? Let's write a simplistic network service that prints out whatever is given to us. We must read from one input stream and accrue everything into a new buffer. Then, when the request finishes, we'll print it out. The problem is we might get a lot of data simultaneously. So, we used threads.
...
And, of course, if you want a native image, there is also the graalvm project, which provides an ahead-of-time (AOT) compiler that will be released today, September 19th, 2023. You can use this to compile your highly scalable Boot applications into GralVM native images that start in no time and take a tiny fraction of the RAM they took on the JVM.
